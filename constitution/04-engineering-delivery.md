...
- v0.3.0 (2026-08-28): added sales↔engineering handoff layer — `scope-intake` (Metis proposal → Kiki review), `delivery-tracker` (weekly sales status), `feasibility-gate` (blocks Metis sends without Kiki sign-off). Addresses the gap that Metis was drafting proposals with no technical input. Total sub-agents: 5 → 8. Synced from canonical `/opt/data/agents/departments/04-engineering-delivery.md` to bring this v2 copy into agreement.
